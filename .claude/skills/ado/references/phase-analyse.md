...
6. **Task-Subagents:** Pro discussion-offenem Task **ohne** `(x)` auf zugeordnetem Description-Punkt und **ohne** effektives `TASK-CLOSED` → `ado-task-pruefe-agent` Modus **`analyse`** ([`task-analyse-subagent.md`](task-analyse-subagent.md))
