# Tool-Übersicht: Alle verfügbaren Tools (43)
...
| Tool | Was es tut |
|------|------------|
| `index_project` | Vollständige Landkarte des Projekts: alle Klassen, Services, Components, Routes, Abhängigkeitsgraph, Architektur-Warnungen. Gecacht 5 min. **NEU:** `projects[]` für Batch-Indexierung mehrerer Pfade in einem Call. |
| `index_solution` | .NET only: kombinierter Index aller Projekte einer `.sln`. Cache 5 min neben der `.sln`. Known Issue bei manchen Solutions. |
| `index_status` | **NEU** — Status aller gecachten Projekte: `indexedAt`, `expiresAt`, `symbolCount`, `stale`. Vor `index_project` prüfen ob Cache warm. |
| `find_in_index` | Gezielt nach Klasse/Service/Interface suchen. **NEU:** `format`: `paths_only` / `compact` / `full` für token-sparende Ausgabe. |
| `scout_symbol` | **NEU COMPOSITE** — Symbol suchen in einem Call: Index → Filesystem-Fallback. `format`: paths_only/compact/full. `fallbackToFilesystem`, `includeReferencesCount`. Ersetzt: index_project → find_in_index → find_by_content. |
| `scout_scope` | **NEU COMPOSITE** — Batch-Scout: `questions[]`-Array in einem Call. Output `scout_table` direkt in Buddy-Protokoll einsetzbar. |
...
| Tool | Was es tut |
|------|------------|
| `analyze_compiler_diagnostics` | Echter Compiler-Check (keine Heuristik): Roslyn (`.NET`) bzw. ts-morph (Angular). Kein Shell-Build. `path`, `type`, `severity` (error\|warning\|all). |

## ⑧ BoyScoutRule

| Tool | Was es tut |
|------|------------|
| `suggest_boyscout_actions` | BoyScout-Orchestrator: `filePaths[]` rein, priorisierte Top-N-Verbesserungsliste raus. Compiler-Gate zuerst; aggregiert Nullability, Dead Code, Complexity, Untested API. `maxPerFile` (Default 5). |

## ⑨ Composite & Domain-Finder (NEU)

| Tool | Was es tut |
|------|------------|
| `analyze_slice_impact` | **COMPOSITE** — Post-Slice: Compiler → BoyScout → Untested API → Refactoring Safety in einem Call. `format`: compact/full. Ersetzt 4+ separate Calls. |
| `analyze_planning_inventory` | Endpoint/Route/DTO-Inventar aus `filePaths[]` oder `projectPath`. `format: compact` = keine Raw-AST. |
| `trace_api_contract` | FE Angular-Service → BE-Endpoint + Validierungs-Delta. 1 Call für „was ruft dieser Service, welche Validierungslücken?" |
| `find_angular_route` | Route-Pfad → Component + Guards. Angular-Index + Filesystem-Fallback. Max 20 Treffer. |
| `find_angular_guard` | Guard-Name → Datei + canActivate-Kette. |
| `find_dotnet_endpoint` | Controller/Action oder Route-Template → Methode + DTO. |
| `find_di_registration` | Service/Interface → Program.cs/Startup-Registrierung (AddSingleton/Scoped/Transient). |
| `find_api_consumers` | Reverse zu `find_api_callers`: BE-Endpoint → alle FE-Call-Sites. |

## Wichtige Parameter

**Alle Pfade = Windows-Absolutpfade** (`C:\Develop\...`) — kein `/workspace/`, kein Docker.

| Parameter | Verwendet von | Format |
|-----------|--------------|--------|
| `projectPath` / `solutionPath` | `index_project`, `index_solution`, `review_with_index`, `find_symbol_references`, `find_type_hierarchy`, `detect_god_classes`, `scout_symbol`, `scout_scope` | `C:\Develop\MyProject\src\frontend` |
| `projects` | `index_project` (Batch) | `["C:\\...\\frontend", "C:\\...\\backend"]` |
| `filePath` | `review_file`, `analyze_ast_only`, `analyze_compiler_diagnostics`, `find_symbol_references` u.a. | `C:\Develop\MyProject\src\app\my.service.ts` |
| `filePaths` | `review_files_batch`, `suggest_boyscout_actions`, `analyze_slice_impact`, `analyze_planning_inventory` | `["C:\\...\\a.ts", "C:\\...\\b.cs"]` |
...
| `format` | `find_in_index`, `scout_symbol`, `scout_scope`, `analyze_slice_impact` | `"paths_only"` \| `"compact"` \| `"full"` \| `"scout_table"` |

**VERBOTEN:** `/workspace/`-Pfade, relative Pfade, `{parameter}`-Platzhalter. Beide MCPs (dev-mcp + codebase-analyzer) nutzen Windows-Absolutpfade.
