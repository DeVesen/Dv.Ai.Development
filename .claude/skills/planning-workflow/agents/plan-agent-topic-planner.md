...
| D | dev-mcp (Pflicht bei bestehenden Klassen / nach Index-Miss): Kanon [docs/mcp/dev-mcp.md](../../docs/mcp/dev-mcp.md); `read_class_summary` / `read_signatures_only` / `find_by_content` mit `file_path`/`root` als Windows-Absolutpfad (`C:\...`) | Read nur nach ausgeschöpfter MCP-Kette | bestehende Klassen im Topic-Scope |
