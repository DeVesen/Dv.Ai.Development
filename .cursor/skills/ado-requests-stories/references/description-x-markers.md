...
3. Punkt **ohne `(x)`** und **ohne** passenden Task → neue `tasks/task-{kebab-slug}.md` aus [task-open.md.template](../templates/task-open.md.template) (minimal: Kopf, `## Story-Bezug`, `## Anforderung`, `## Möglichkeiten`).
...
```markdown
> Description `(x)` = PM markiert erledigt — siehe `## Description-Analyse (ADO (x))` und [description-x-markers.md](references/description-x-markers.md).
```
