---
name: describe-as-html-prompt
description: >
  Like describe-as-prompt but emits a single copy-paste-ready standalone HTML handoff
  document for a follow-up agent. Same Section A complexity note + Section B payload,
  watertight mode vs planning obligation rules, thread fidelity. Section B must embed
  Mermaid sequenceDiagram(s) for cross-layer/service flows discussed (FE↔BE, service A→B)
  and Mermaid classDiagram(s) (or equivalent Mermaid) for models/types with methods and
  notable functions when the thread supplies them. Triggers: describe-as-html-prompt rule —
  u.a. beschreibe als HTML, HTML-Prompt, in HTML zusammenfassen, html prompt für.
...
# Describe-as-HTML-Prompt (conversation to HTML handoff)

Portable skill: mirror of **describe-as-prompt** with **HTML + Mermaid** as the handoff artifact. Reuse the same **Planning Workflow** / **Wasserdicht** logic from [`describe-as-prompt/SKILL.md`](../describe-as-prompt/SKILL.md) unless this file explicitly overrides it — **only** output format and **Mermaid Pflichten** differ.

## Purpose

After an exploratory discussion, produce **one** assistant-visible **Section A** (short complexity note, same rules as describe-as-prompt) plus **Section B** as a **complete standalone HTML document** the user can save as `.html` or paste elsewhere. The HTML must be **self-contained** regarding rendering: include a **public CDN** load for Mermaid (see **HTML shell** below) so opening the file in a browser diagrams render without a build step.

## Before you respond

1. Read **this** `SKILL.md` fully; for **Wasserdicht**, **Planning obligation**, **Section A** tier table, and **Section B** semantic headings, align with [`describe-as-prompt/SKILL.md`](../describe-as-prompt/SKILL.md) (same priority order: Wasserdicht → planning-relevant → pure information).
2. Derive content **only** from the **current conversation** — same fidelity rules as describe-as-prompt (paths, code refs, no invented repo facts).
...
Same **two-part order** as describe-as-prompt:

### Section A — Short complexity note

Identical requirements to describe-as-prompt (`SKILL.md`): coarse complexity, planning-overview tier **unless** Wasserdicht or non-planning handoff, disclaimer.

### Section B — Handoff as standalone HTML

Wrap the **entire HTML document** in **one** fenced code block with language tag **`html`**.

**Inside the HTML**, mirror the **semantic sections** from describe-as-prompt Section B as `<h2>` / `<section>` (German headings OK if user language is German):
...
9. **Planning obligation** — only when describe-as-prompt mandates it; never in Wasserdicht.
...
Same as describe-as-prompt: executable handoff, thread fidelity, no invented code.
...
Repositories indexing skills in `{agent-index}` should reference this skill alongside describe-as-prompt when users want **HTML + Mermaid** handoffs.

**Trigger-Pflege:** YAML `description` + [.cursor/rules/describe-as-html-prompt-skill.mdc](../../rules/describe-as-html-prompt-skill.mdc).
