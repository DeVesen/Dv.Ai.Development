# Build.Log.Filter — MCP Output-Filter (C# / .NET 9)
...
```bash
dotnet run --project src/Build.Log.Filter
```
...
```json
{
  "mcpServers": {
    "Build.Log.Filter": {
      "command": "dotnet",
      "args": ["run", "--project", "src/Build.Log.Filter/Build.Log.Filter.csproj"]
    }
  }
}
```
...
Anpassung: `FilterLimits` in [Program.cs](src/Build.Log.Filter/Program.cs) registrieren.

## Architektur

- Parser unter `src/Build.Log.Filter/Filtering/`  
- Streaming unter `src/Build.Log.Filter/Streaming/`  
- MCP-Tools unter `src/Build.Log.Filter/Tools/`  
