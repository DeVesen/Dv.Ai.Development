...
Verfügbaren MCP situativ wählen. **MCP-Sequenz** gemäß [repo-scout-protocol/SKILL.md](../skills/repo-scout-protocol/SKILL.md) vor Symbol-Auflösung — Read/Grep erst nach ausgeschöpfter Kette. Default: `codebase-analyzer`.
...
| D | dev-filesystem-mcp (Pflicht bei bestehenden Klassen / nach Index-Miss): Kanon [docs/mcp/dev-filesystem.md](../../../docs/mcp/dev-filesystem.md); `read_class_summary` / `read_signatures_only` / `find_by_content` mit `file_path`/`root` unter `/project/...` | Read nur nach ausgeschöpfter MCP-Kette | bestehende Klassen im Topic-Scope |
...
6. **Pflicht — Vorgeschlagene IMP-Slice-IDs:** gemäß [SKILL.md](../skills/planning-workflow/SKILL.md) **Slice-ID-Konvention** — `IMP-FE-{Bereich}-…` bzw. `IMP-BE-{ServiceKürzel}-…` plus Wellen-/Blocking-Hinweis; ohne Gesamtplan.
