...
- **`## Story-Bezug`:** relevante Auszüge aus der **Story-**Description (und ggf. `## Feature-Kontext`), die diesen Task inspirieren — wörtlich oder als Zitatblock.
...
- Bestehende Task-Dateien: `## Umsetzung`, `## Nutzer-ToDos` **nicht** überschreiben — nur Kopfzeile `**Status:**` und Story-Checkboxen bei Marker-Sync.

## Slug aus Titel

1. Titel bereinigen (Sonderzeichen entfernen)
2. Kebab-case für `task-{slug}.md`
3. Story-Ordner: `UserStory-{id}-{PascalOrKebabFromTitle}`
