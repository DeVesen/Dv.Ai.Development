...
**Auch laden:** `.claude/skills/angular-developer/SKILL.md` für Language-Level-Guidance (Components, DI, Routing, CLI).
...
3. `angular-developer` — Components, Signals, Forms, DI, Routing, Tests, CLI.
...
**Projekt-Overrides:** Repo-Regeln (Tailwind-Verbot, Pflicht-UI-Lib, abweichende Ordnernamen wie `shared/ui`) ranken über Vendor-Skill. `pages/` ist eine empfohlene Konvention, kein Pflichtordner — wenn das Repo routete Komponenten anders ablegt, diesem Muster folgen.
...
- Nach substantiellen Edits: `build_angular_project` via dev-angular-mcp ausführen und Compile-Fehler beheben.
