# Operation: Generate Commit Message (context-based)

Generate a **commit-ready** English title and body from **current context** — conversation thread, cited files, optional local git state — without executing `git commit` unless the user explicitly asks.
...
1. Derive content **only** from allowed sources below. Do **not** invent changes not supported by context.
2. If the scope is unclear, ask **one** focused question **or** mark gaps as `(needs clarification)` in the body — do not hallucinate file names or behavior.
...
3. **Local git state** — only when the user clearly wants a message for **uncommitted work**: read-only `git status` / `git diff`. **Do not** run `git commit`.
...
## Abgrenzung (ADO / ado-agent)

**Not this operation** when the user uses the formal copy line:

`Commit-Vorschlag für Task {taskDateistamm} in Story {storyId}`

That flow stays in the ado skill and **ado-agent**: Task-MD sources, Description max. **400** characters, **must** include `Story #{storyId}`.
