...
**Ausgabe-Stil aller Handoffs: MACHINE-DENSE** — Kein Fliesstext, keine Rollenwiederholung, Key:Value wo ausreichend. Deliverables zurueck an Orchestrator: MACHINE-DENSE.

**Agent-Typ (Pflicht):** Profil unter `.claude/agents/`. **Modell:** Agent-Profil lesen; primaer `## Modell`; Slugs nicht in Prompts duplizieren.

**Compliance (Pflicht):** Vor jedem Subagent `subagent-delegation-boilerplate.md` in den Task-Prompt.

Vorlagen sind **Auftrags-Payloads** (Platzhalter) — kein Ersatz fuer Agent-Profile.
...
MCP-Pfade (aus Projekt-Dokumentation — vor Versand Literale eintragen):
  FE: [MCP_FRONTEND_PATH]   (= MCP container path FE — /workspace/... fuer codebase-analyzer)
...
Schritt 1 — Basis-Landkarte (Pflicht):
   ...
   index_project je BE-Projektpfad, type="dotnet" fuer BE — nicht pauschal Backend-Root.
...
Deliverable:strukturierte Aufzaehlung — kein Code, kein Umsetzungsplan. Stil:MACHINE-DENSE.
```
...
Ein Lauf mit Agent-Typ **`plan-agent-interface-designer`** — nach Scout-Zusammenfuehrung, **vor** Phase 4b. Kein Parallelstart mit Topic-Planern.

```text
Profil:plan-agent-interface-designer|Topic-Map+Schnittstellen-Vertrag|kein-Gesamtplan|kein-Review|keine-Teilplaene

Scout-Merge (Pflicht — alle Scout-Deliverables, Positionen 0–10 je Scout):
[Zusammenfuehrung aller Scout-Rueckgaben: MCP-Status, Dateien, Einstiegspunkte, Nachbarschaft, Risiken, Luecken, Hotspots]
...
4. **Offene Punkte:** Unaufgeloeste Scout-Findings, die den Schnittstellen-Vertrag
   beeinflussen — explizit markieren.

MCP-Nachverifikation (nur bei konkreten Luecken im Schnittstellen-Vertrag):
   Fehlende Signatur oder unbekannter Typ → find_in_index mit projectPath aus
   Projekt-MCP-Pfad-Dokumentation. Ergebnis (ok/fallback) festhalten.

Deliverable:Topic-Map + Schnittstellen-Vertrag (+ Sequence-Diagramm bei >= 2 Topics) — kompakt, vollstaendig. Stil:MACHINE-DENSE.
```
...
find_in_index versuchen (projectPath aus Projekt-MCP-Pfad-Dokumentation / Routing-Tabelle);
Ergebnis (ok oder fallback) im Teilplan festhalten — kein stilles Ueberspringen.

Deliverable:strukturierter Teilplan fuer genau ein Topic — kein Code, kein Gesamtplan, kein Review anderer Topics. Stil:MACHINE-DENSE.
```
...
Ein Lauf mit Agent-Typ **`plan-agent-merger`** — nach Abschluss aller Topic-Planer aus 4b, **vor** Phase 5.

```text
Profil:plan-agent-merger|merge-Teilplaene→Arbeitsversion|kein-Scout|kein-Review|keine-Impl

Vertrag (Phase 4a — vollstaendig):
[Topic-Map + Schnittstellen-Vertrag + Sequence-Diagramm aus Interface-Designer-Deliverable]

Teilplaene (Phase 4b — alle):
[Alle plan-agent-topic-planner-Deliverables — je Topic vollstaendig]

Req:[Bullets — Auszug Phasen 1–2, max. 5]

Aufgabe:

1. Fuehre alle Topic-Teilplaene zu einer **Arbeitsversion** zusammen (startfaehig
   fuer Phase 5 ohne weitere Recherche).

2. **Drift-Pruefung (Pflicht):** Schnittstellen aus Phase 4a vs. Teilplaene —
   Abweichungen, Luecken, Widersprueche aufloesen oder als Nutzerfrage markieren.

3. Gesamtuebersicht: relevante Dateien, Einstiegspunkte, Schritte,
   Akzeptanzkriterien, Risiken, offene Fragen.

4. **IMP-Slices ableiten:** Aus den vorgeschlagenen IMP-Slice-IDs der Topic-Teilplaene
   (Schritt 6 je Topic) eine konsistente Slice-Tabelle zusammenfuehren. Keine neuen
   Slices erfinden — nur aus Teilplan-Deliverables uebernehmen, Konflikte aufloesen,
   Duplikate bereinigen.

5. **Multi-Subagent-Aufteilung:** Arbeitspakete, Parallelitaet, Blocking, gemeinsame
   Artefakte, Interface-first, Orchestrator-Integration, E2E-Pruefung — oder
   Begruendung Single-Agent.

6. **Wellen und Blocking** fuer Phase 6 vorbereiten
   (W0 contract-first, W1 parallele Slices, W2 Integration).

Deliverable:vollstaendige Arbeitsversion — kompakt, konsistent, Phase-5-bereit. Stil:MACHINE-DENSE.
```
...
Plan:[Arbeitsversion aus Phase 4c — vollstaendig]
...
Stil:BULLET-TERSE. Nummerierte Punkte. Kein neuer Plan; nur Risiken und Luecken.
```
...
Stil:BULLET-TERSE. Nummerierte Punkte. Kein neuer Plan; nur Ausfuehrbarkeit und Detailtiefe.
```
...
Abschluss:Gesamtnote 1–6 + kurze Begruendung.
...
Abschluss:Gesamtnote 1–5 + ausfuehrliche Begruendung. Mindestens 5 Punkte.

Stil:BULLET-TERSE. Priorisierte Maengelliste, dann Note. Kein neuer Plan; nur Pruefergebnis.
...
Ein Lauf mit Agent-Typ **`plan-agent-synthesizer`** — nach Abschluss aller fuenf Phase-5-Reviews. Kein Parallelstart.
...
Arbeitsversion (Phase 4c — vollstaendig):
[Merger-Deliverable]

Reviews (Phase 5 — alle fuenf):
Optimist:[vollstaendig]
Pessimist:[vollstaendig]
Normalo:[vollstaendig]
Oberlehrer:[vollstaendig inkl. Note]
Professor:[vollstaendig inkl. [KRITISCH]-Punkte und Note]
...
Schritt 1 — Review-Digest (zuerst ausgeben, vor inhaltlicher Synthese):
   Fuenf Abschnitte: Optimist, Pessimist, Normalo, Oberlehrer, Professor.
   Pro nummeriertem Punkt max. 1–2 Saetze Kernaussage.
...
Schritt 3 — Komplexitaets- und Executor-Empfehlung:
   Low/Medium/High, Executor-Tier illustrativ, Topologie-Hinweis (konsistent mit
   Umsetzungs-Topologie), Begruendung 2–4 Saetze, Disclaimer.
   Bei trivialem Plan einzeilig "Empfehlung nicht erforderlich".

Schritt 4 — Finales Planpaket:
   Vollstaendigen Freigabetext formulieren. Pflicht-Abschnitt Umsetzungs-Topologie
   (Implementation Workflow) gemaess SKILL.md Phase 6 — oder Trivial-Kurzform.
   Ohne diesen Abschnitt (wenn Implementierung vorgesehen): Planpaket unvollstaendig.

Deliverable:Review-Digest + Synthese-Checkliste + Komplexitaets-Block + finales Planpaket — kompakt, konsistent, freigabefertig. Stil:MACHINE-DENSE fuer Agent-Payloads; Review-Digest + Planpaket BULLET-TERSE (User-sichtbar).
```
...
8. **Finales Planpaket:** Vollstaendigen Freigabetext formulieren (integriert
