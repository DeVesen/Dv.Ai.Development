...
- MCP-Server **`ado`** erreichbar — Paket `@azure-devops/mcp`
...
**VERBOTEN:** Artefakte unter Skill-Pfad, `.claude/` oder Code-Unterverzeichnissen anlegen.
...
**Task-Klärung / Planung:** **nicht** ADO — Nutzer nutzt `buddy intake …` / `buddy repo-check …` (Copy aus Task-`## Möglichkeiten`).
...
| `analyse` Feature-Kaskade | `ado-story-pruefe-agent` (parallel, max. 10) | Agent-Profil im Projekt |
| `analyse story` (direkt) | Orchestrator selbst | [`story-analyse-subagent.md`](references/story-analyse-subagent.md) |
| Task-Drafts je offenem Task | `ado-task-pruefe-agent` Modus `analyse` | Agent-Profil im Projekt |
...
| Intent | Skill |
|--------|-------|
| `load` / `analyse` / `save` | **dieser Skill** |
...
| Situation | Vorrang |
|-----------|---------|
| `buddy intake` / `buddy repo-check` / Plan-Prompt | **buddy-agent** |
| `load` / `analyse` / `save` | **ado** |
| `plane Task …` | Planning Workflow |
| `implementiere …` | Implementation Workflow |

### `Task … verfeinern` (Legacy)

| Situation | Aktion |
|-----------|--------|
| Buddy / Sparring / Plan-Prompt | → buddy-agent, Copy `buddy intake …` |
