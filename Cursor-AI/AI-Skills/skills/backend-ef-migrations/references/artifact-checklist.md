...
|---------|-------------------------|----------|
| `Npgsql.PostgresException: 42703: column … does not exist` auf `{view-name}` | View in DB ohne neue Spalte; Migration nicht angewendet oder View-SQL fehlt in `Up()` | `migrations list` / `__EFMigrationsHistory`; View-SQL prüfen; `database update` oder Service-Neustart |
...
- Betrifft die Änderung `{view-name}` auch `{ViewEntity}` und Search-Service-Spalten?
