...
|-----------|-------------|
| `{backend-path}` | Pfad zum Backend-Projekt |

# EF CLI — {database-project-name}
...
`{startup-project-name}` ruft beim Start `db.Database.Migrate()` auf (`{backend-path}/{startup-project-name}/Program.cs`). Das wendet alle ausstehenden Migrationen der EF-Kette an — **nicht** handgeschriebene Dateien ohne Designer-Eintrag in der Kette.
