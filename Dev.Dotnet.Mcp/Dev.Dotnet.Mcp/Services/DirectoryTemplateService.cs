...
public sealed class DirectoryTemplateService
{
    public DirectoryStructureResult Create(string basePath, string pathsJson)
    {
        if (string.IsNullOrWhiteSpace(basePath))
            return Fail("base_path is required.");
        if (string.IsNullOrWhiteSpace(pathsJson))
            return Fail("paths_json is required.");

        string[] paths;
        try
        {
            paths = JsonSerializer.Deserialize<string[]>(pathsJson, JsonDefaults.Options)
                ?? throw new JsonException("Expected JSON array of paths.");
        }
        catch (Exception ex)
        {
            return Fail($"Invalid paths_json: {ex.Message}");
        }
        ...
        try
        {
            ...
            foreach (var relative in paths)
            {
                ...
                if (relative.Contains("..", StringComparison.Ordinal))
                    return Fail($"Path traversal denied in entry: {relative}");
            ...
            {
                Success = true,
                CreatedDirs = createdDirs.Distinct(StringComparer.OrdinalIgnoreCase).ToList(),
    ...
    private static DirectoryStructureResult Fail(string message) => new()
    {
        Success = false,
        Error = message
    };
