...
public class FilterCallHistoryTests
{
    ...
    public void Ringbuffer_Drops_Oldest_When_Over_Max()
    {
        ...
        var entries = history.GetLast(100);

        Assert.Equal(20, entries.Count);
        // Newest should be Tool25, oldest kept should be Tool6
        Assert.Equal("Tool25", entries[0].ToolType);
        Assert.Equal("Tool6", entries[19].ToolType);
    }
