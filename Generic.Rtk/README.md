# Dv.Ideas.Generic.Rtk — MCP Output-Filter (C# / .NET 9)
...
```bash
dotnet run --project src/Generic.Rtk
```
...
{
  "mcpServers": {
    "Dv.Ideas.Generic.Rtk": {
      "command": "dotnet",
      "args": ["run", "--project", "src/Generic.Rtk/Generic.Rtk.csproj"]
    }
  }
}
```
...
```bash
docker build -t Dv.Ideas.Generic.Rtk-mcp .
docker run -i --rm Dv.Ideas.Generic.Rtk-mcp
```
...
Anpassung: `FilterLimits` in [Program.cs](src/Generic.Rtk/Program.cs) registrieren.

## Architektur

- Parser unter `src/Generic.Rtk/Filtering/`  
- Streaming unter `src/Generic.Rtk/Streaming/`  
- MCP-Tools unter `src/Generic.Rtk/Tools/`  
