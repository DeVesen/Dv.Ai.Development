...
import { formatRoslynSection } from "./analyzers/roslyn-formatter.js";
...
function performReview(
  ...
  if (language === "angular") {
    ...
    const isNonTrivialCSharp = (code ?? '').split('\n')
      .filter(l => l.trim() && !l.trim().startsWith('//') && !l.trim().startsWith('*'))
      .length > 5;
    let meta = analyzeCSharp(code, filename);
    if (!meta.error && meta.classes.length === 0 && isNonTrivialCSharp) {
      meta = { ...meta, error: 'AST empty — Roslyn parse failed, review inconclusive' };
    }
