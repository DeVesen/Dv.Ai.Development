...
public sealed class CodeReadService
{
        ...
        foreach (var member in root.DescendantNodes().OfType<MemberDeclarationSyntax>())
        {
            switch (member)
            {
                case MethodDeclarationSyntax method:
                    if (!includePrivate && IsPrivate(method.Modifiers))
                        continue;
                    results.Add(new SignatureEntry(
                        "method",
                        GetAccess(method.Modifiers),
                        method.WithBody(null).WithSemicolonToken(default).ToFullString().Trim(),
                        method.GetLocation().GetLineSpan().StartLinePosition.Line + 1));
                    break;

                case PropertyDeclarationSyntax property:
                    if (!includePrivate && IsPrivate(property.Modifiers))
                        continue;
                    results.Add(new SignatureEntry(
                        "property",
                        GetAccess(property.Modifiers),
                        property.ToFullString().Trim().TrimEnd('{').Trim(),
                        property.GetLocation().GetLineSpan().StartLinePosition.Line + 1));
                    break;

                case IndexerDeclarationSyntax indexer:
                    if (!includePrivate && IsPrivate(indexer.Modifiers))
                        continue;
                    results.Add(new SignatureEntry(
                        "indexer",
                        GetAccess(indexer.Modifiers),
    ...
    private static MethodReadResult ReadCSharpMethod(string filePath, string methodName, string? className)
    {
        ...
        foreach (var typeDecl in root.DescendantNodes().OfType<TypeDeclarationSyntax>())
        {
            ...
            {
                if (!method.Identifier.Text.Equals(methodName, StringComparison.Ordinal))
                    continue;

                var span = method.GetLocation().GetLineSpan();
                var startLine = span.StartLinePosition.Line + 1;
                var endLine = span.EndLinePosition.Line + 1;
                var signature = method.WithBody(null).WithSemicolonToken(default).ToFullString().Trim();
                var body = method.Body?.ToFullString().Trim()
                           ?? method.ExpressionBody?.ToFullString().Trim()
                           ?? string.Empty;

                return new MethodReadResult(signature, body, startLine, endLine);
            }
        }
    ...
    private static ClassSummaryResult ReadCSharpClassSummary(string filePath, string? className)
    {
        ...
        if (typeDecl.BaseList is not null)
        {
            ...
            {
                if (LooksLikeInterface(baseTypes[0]))
        ...
        var properties = typeDecl.Members.OfType<PropertyDeclarationSyntax>()
            .Select(p => new ClassPropertySummary(
                p.Identifier.Text,
                p.Type.ToString(),
                GetAccess(p.Modifiers)))
            .ToList();
    ...
    private static IReadOnlyList<SignatureEntry> ReadTypeScriptSignatures(string filePath, bool includePrivate)
    {
        ...
        for (var i = 0; i < text.Length; i++)
        {
            ...
            if (propMatch.Success && !line.Contains('('))
            {
                ...
                if (!includePrivate && access == "private")
                    continue;
            ...
            if (!includePrivate && methodAccess == "private")
                continue;
    ...
    private static MethodReadResult ReadTypeScriptMethod(string filePath, string methodName, string? className)
    {
        ...
        for (var i = 0; i < lines.Length; i++)
        {
            ...
            {
                if (Regex.IsMatch(line, $@"class\s+{Regex.Escape(className)}\b"))
                    inClass = true;
                continue;
            }
    ...
    private static ClassSummaryResult ReadTypeScriptClassSummary(string filePath, string? className)
    {
        ...
        {
            classMatch = TsClassRegex.Match(text);
        }
        else
        {
        ...
        if (!classMatch.Success)
            throw new InvalidOperationException("No class found in file");
    ...
    private static string GetAccess(SyntaxTokenList modifiers)
    {
