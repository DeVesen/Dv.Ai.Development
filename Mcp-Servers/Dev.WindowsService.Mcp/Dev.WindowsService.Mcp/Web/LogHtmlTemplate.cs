namespace Dev.WindowsService.Mcp.Web;
...
internal static class LogHtmlTemplate
{
    public static string GetHtml() => """
        ...
        <head>
          ...
          <title>Dev.WindowsService.Mcp — Log</title>
          <style>
            *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .filter-btn {
              ...
              background: #1e2228; color: #64748b; border-color: #1e2228;
            }
            .filter-btn.active { color: #fff; }
            .filter-btn[data-src="all"].active  { background: #1e40af; border-color: #3b82f6; color: #bfdbfe; }
            .filter-btn[data-src="filesystem"].active { background: #065f46; border-color: #10b981; color: #a7f3d0; }
            .filter-btn[data-src="dotnet"].active { background: #4c1d95; border-color: #8b5cf6; color: #ddd6fe; }
            .filter-btn[data-src="angular"].active { background: #7f1d1d; border-color: #ef4444; color: #fecaca; }
            ...
            .btn-clear {
              ...
              padding: 5px 14px; border-radius: 6px; font-size: 0.8rem; cursor: pointer; transition: background .15s;
            }
            ...
            .item { background: #111318; border: 1px solid #1e2228; border-radius: 8px; margin-bottom: 8px; overflow: hidden; }
            .item-header {
            ...
            .src-tag {
              font-size: 0.68rem; font-weight: 700; padding: 2px 8px; border-radius: 99px; white-space: nowrap; flex-shrink: 0;
            }
            .src-filesystem { background: #065f46; color: #a7f3d0; }
            .src-dotnet     { background: #4c1d95; color: #ddd6fe; }
            .src-angular    { background: #7f1d1d; color: #fecaca; }
            ...
            .section-title { font-size: 0.72rem; font-weight: 700; letter-spacing: .06em; text-transform: uppercase; color: #64748b; }
            .btn-copy {
              background: none; border: 1px solid #374151; color: #64748b;
              border-radius: 4px; width: 20px; height: 20px; padding: 0; cursor: pointer;
              display: inline-flex; align-items: center; justify-content: center; transition: border-color .12s, color .12s; flex-shrink: 0;
            }
        ...
        <body>
          <header>
            <h1>&#x25CF; Dev.WindowsService.Mcp</h1>
            <span class="badge" id="count">0 Calls</span>
            <div class="filter-group">
              <button class="filter-btn active" data-src="all" onclick="setFilter('all', this)">All</button>
              <button class="filter-btn" data-src="filesystem" onclick="setFilter('filesystem', this)">Filesystem</button>
              <button class="filter-btn" data-src="dotnet" onclick="setFilter('dotnet', this)">.NET</button>
              <button class="filter-btn" data-src="angular" onclick="setFilter('angular', this)">Angular</button>
            </div>
          ...
          <script>
            ...
            function copyBtn(text) {
              return '<button class="btn-copy" title="Kopieren" onclick="copyText(this)" data-copy="' + esc(text) + '">' + COPY_SVG + '</button>';
            }
            ...
            function render() {
              var items = activeFilter === 'all' ? allItems : allItems.filter(function(it) { return it.source === activeFilter; });
            ...
            function load() {
              fetch('/api/calls')
                ...
                .then(function(data) { if (data) { allItems = data; render(); } })
                .catch(function() {});
            }
