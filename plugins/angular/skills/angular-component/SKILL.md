...
### Binding collisions & static host attributes

If both the component's `host` binding and the consumer's template binding target the same property: static vs static → consumer wins; static vs dynamic → the dynamic one wins; dynamic vs dynamic → the component's host binding wins.

Read a static attribute the host was given, at construction time, via `HostAttributeToken`:

```typescript
type = inject(new HostAttributeToken('type'));   // throws if missing, unless { optional: true }
```
...
### Multi-case `@switch` (v21.1+)

Consecutive `@case` blocks can share one body — the empty case falls through to the next:

```html
@switch (status()) {
  @case ('pending') {}
  @case ('active') { <span>In progress</span> }
  @default { <span>Unknown</span> }
}
```

### Template expression spread (v21.1+)

Template expressions support spreading objects/arrays and spread args in function calls: `<app-card [config]="{...base, extra: 1}" />`.

### Deferred loading (`@defer`)

Split a template into an eagerly-rendered shell and a lazily-loaded, separately-bundled chunk:
...
Triggers: `on idle` (default), `on viewport`, `on interaction`, `on hover`, `on timer(Xms)`, `on immediate`, or `when <expr>`. Separate multiple triggers with `;` — as OR conditions between render triggers (`on viewport; on timer(5s)`), or to pair a render trigger with a `prefetch` trigger that starts loading the chunk earlier without rendering it yet (`on interaction; prefetch on idle`).
