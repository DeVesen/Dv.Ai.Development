...
**CRITICAL: never use effect to propagate state.** Calling `.set()`/`.update()` on a signal inside an effect to keep two signals in sync causes `ExpressionChangedAfterItHasBeenChecked` errors and infinite loops — use `computed()` or `linkedSignal()` for derivation instead.

**Async gotcha:** the reactive context only tracks synchronous reads — a signal read after `await` inside an effect is NOT tracked. Read all signals before the `await`:

```typescript
// WRONG — theme() read after await, not tracked
effect(async () => { const data = await fetchUserData(); console.log(theme()); });
// RIGHT — read before the await
effect(async () => { const currentTheme = theme(); const data = await fetchUserData(); console.log(currentTheme); });
```

DOM reads/writes based on a signal change (e.g. 3rd-party UI library integration) → `afterRenderEffect` (runs after Angular renders, client-only) instead of plain `effect` (runs before DOM update): [references/after-render-effect.md](references/after-render-effect.md).
...
For advanced patterns including resource(), see [references/signal-patterns.md](references/signal-patterns.md). If the app uses Angular's `HttpClient`, prefer `httpResource` over raw `resource()` + `fetch` — same signal API, uses the HTTP stack including interceptors.
