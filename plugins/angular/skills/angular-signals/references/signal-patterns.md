...
### httpResource — HttpClient integration

Prefer this over raw `resource()` + `fetch` whenever the app already uses `HttpClient` — same signal API, goes through the HTTP stack (interceptors, auth, etc.):
...
export class UserProfile {
  ...
  // Re-fetches whenever the returned URL changes
...
Same status API as `resource()` (`value()`, `hasValue()`, `error()`, `isLoading()`, `status()`, `reload()`). Return `undefined` from the request function to skip loading, same as `params` returning `undefined` for `resource()`.
...
export class Search {
  ...
  // Derived loading state — NOT synced via effect() (see CRITICAL rule above).
  // True while a debounced query hasn't produced results yet.
  ...
  readonly isSearching = computed(() =>
    this.query().length >= 2 && this.query() !== this.debouncedQuery()
  );
}
```
