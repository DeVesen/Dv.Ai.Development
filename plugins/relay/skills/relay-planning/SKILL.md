...
**Where the artifacts live.** Default artifact home:
`docs/relay/<request-id>-<slug>/`, with `<request-id>` defaulting to
`YYYY-MM-DD` from the date already present in your environment — never computed
or invented. This is a **default**, and so is the language the artifacts are
written in. Where the project's always-loaded instruction file names an artifact
home or an artifact language in its `## Relay process discipline` section —
installed by `relay-init` — that binds. Absent it, use the path above and the
language of this conversation, say in your first message which language you are
writing in, and never infer it from files that already exist. Anything quoted
from the requester is recorded in the words they used, never translated.

Then, before anything else, check whether this request has a relay history at all:
...
| No `02-spec.md` at all | No plan. Name `relay-refining` and stop — the spec is what is missing, not its review. |
| No `03-spec-review.md` at all, but `02-spec.md` exists | No plan. Name `relay-reviewing-spec` and stop. |
...
draft check and the `04-plan.md` template all apply exactly as written.
...
check needs does not exist yet. Write it yourself, as a new section of
the plan:
...
four slots, code in steps resting only on facts actually read, and the single
draft pass of *Check Your Own Draft, Once*. The forbidden list
...
## The Spec Is the Planning Input

Read `02-spec.md` from disk, and never plan from live session memory of it.
That is the plan's source and the only file this stage reads for content. The
gate has already read `03-spec-review.md` for its verdict; a re-entry reads
`05-plan-review.md` for its findings. Nothing else is a standing read — not
`01-intake.md`, not `00-journal.md`, not a sweep of the codebase. Say in your
first message that you are planning from `02-spec.md` and name anything the
gate found missing.

**Do not go hunting.** If the plan needs a value, a path, an interface or a
sample that `02-spec.md` does not carry, that is one question to a named
person, answered before the plan is written — not a reading expedition, and
never a task at the front of the plan. *Facts You Can Read, Decisions Only a
Person Can Make* tells you which of the two you are holding.

**Open a file only when a specific line you are about to write needs it** — the
exact path going into a task's *Files*, the exact signature going into a
*Consumes*. Open that file, not its neighbours, and only when you are about to
commit it to the plan. A file list that was invented rather than checked is a
guess about the structure, and every task built on it inherits the guess; the
fix is checking the paths you write, not reading the repository first.
...
**Yes — it is a fact.** Get it before you write the plan: one targeted read if
the file is at hand, otherwise one question to the person who has it. Not a
survey, and not a scan of the repository hoping to meet the answer. Every
...
"every code block has to be re-typed". Either way the answer arrives before the
plan is written, never as a task at the front of it.
...
Run these three checks on the finished draft. **This is a pass you run, not a
section you write.** Nothing about it goes into `04-plan.md`: `relay-reviewing-plan`
is required to derive coverage from the actual cards and is explicitly forbidden
to take a table or a self-check line in the plan as evidence anything was checked.
A written one would be work with no reader.

**1. Coverage, both directions.** Walk the spec's `## Behaviour` **and its
`## Acceptance`**, and for each point to the task that carries it. Acceptance is
the half that gets dropped: two of four subjects checked requirements only, and the
acceptance entries that had no task were the phone-readability check and the
end-to-end rehearsal — the two things the requester would have run first. Anything
you cannot point at is one of exactly two things:
...
Then the cheap reverse pass: a task carrying no requirement is work nobody asked
for. Remove it or say what it serves.

**2. Placeholders.** The read-it-alone test on every card — see *And the rule
underneath, because that list is not the list*.

**3. Names match.** Every task's **Consumes** matches an earlier task's
**Produces**, character for character, and the signatures the cards' code
actually defines and calls agree with them. A name that appears in a *Consumes*
and in no *Produces* — or that types one way in the producing card and another
in the consuming one — is a reference to something no task defines.

**Then stop.** Fix what you found, inline, and move on. No second pass, no
re-running the checks against the repaired draft, and no writing down that you
ran them. An independent reviewer runs next; a planner re-reading their own plan
a third time is buying nothing they can catch.
...
The plan ends with the last task. There is no coverage table and no self-check
section in it — see *Check Your Own Draft, Once* for why those are a pass you
run rather than text you write.
...
3. **Then run the whole plan back to the bar**, not just the named findings: the
   coverage check in both directions, the read-it-alone test on every task, and
   *Consumes* against *Produces* — the one pass of *Check Your Own Draft, Once*. A reviewer names what they caught. If you find a
...
one row is the structural decision, and a one-task plan still gets the draft
pass run over it.
...
- Reaching the end without having pointed every acceptance entry at a card.
- Assuming standalone without listing the directory first.
- Reading the codebase, the intake or the journal as a warm-up before planning.
- Writing a coverage table or a self-check section into `04-plan.md`.
...
A full narrative — an approved spec, the sharp reading that finds a
requirement-level gap, and the handoff that replaces the plan — is in
`references/worked-example.md`. Read it when you want the shape of a correct
refusal; it is not needed to write a plan.
